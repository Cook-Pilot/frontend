...
완료 처리 중에는 중복 완료를 막고, draft 저장을 시작하기 전에 실행 중 타이머를
즉시 일시정지하고 예약·활성 알람 취소를 시작한다. 완료 중 늦게 끝난 알람 초기화나
타이머 상태 콜백도 알람을 다시 예약하거나 만료 신호를 보내지 않는다. 최초 완료
요청에서 draft 문맥을 고정한 뒤에는 저장 성공 또는 실패 여부와 무관하게
타이머·단계·도움 행동을 잠근다. 저장 실패 뒤에도 정지된 타이머와 frozen draft를
유지해, 같은 완료 동작으로 안전하게 재시도할 수 있다.
...
개인 버전 승인 API는 backend #37의 새 엔드포인트 계약을 전제로 한다. 기존
개인 버전을 사용자 범위로 조회하기 전에 후기 소유권을 확인하던 ownership P1과
입력 validation 지적은 backend #37에서 수정됐고 CI도 통과했다. 서버 PR의
승인·병합과 배포가 끝난 뒤 프론트 흐름을 함께 활성화한다.
...
- [x] 실행 중 타이머는 완료 시작 즉시 정지·알람 취소되고, draft 저장 지연·실패나
  늦은 알람 초기화 뒤에도 예약 또는 만료 신호가 다시 발생하지 않는다.
