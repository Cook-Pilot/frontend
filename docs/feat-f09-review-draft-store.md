...
SharedPreferences의 플랫폼 쓰기가 `false`를 반환하거나 예외를 던지면
선반영된 메모리 캐시를 디스크 상태로 다시 불러온 뒤 오류를 전달하므로,
미저장 초안이 다음 `load`에서 복구값처럼 보이지 않는다.
...
- 플랫폼 저장 실패 뒤 메모리 캐시와 실제 저장값의 재동기화
