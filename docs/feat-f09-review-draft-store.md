...
초안이 사라진 것처럼 보이지 않는다. 디스크 reload도 실패하면 작업 전 raw
cache 값을 타입별 setter 또는 remove의 선반영 동작으로 best-effort 복원한다.
reload와 fallback 오류는 최초 저장·삭제 오류와 stack을 가리지 않는다.
reload가 성공한 경우에는 작업 전 cache 대신 디스크 상태를 신뢰한다.
...
reload까지 실패하면 제거 전 raw cache 값을 복원해 다음 `load`에서 정리를
다시 시도한다. 정상 `save`와 명시적 `clear`의 저장소 오류뿐 아니라 저장소를
열지 못한 `load` 오류도 호출자에게 전달한다. 따라서 후속 화면은 저장소
장애를 `복구할 초안 없음`으로 오인하지 않고 재시도를 제공할 수 있다.
...
- reload와 fallback 복원까지 실패한 save·clear의 작업 전 cache 및 최초 오류 보존
- 손상값 remove와 reload가 함께 실패한 뒤 다음 load의 cleanup 재시도
