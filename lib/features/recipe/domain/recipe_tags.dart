...
///
/// 식약처 원본의 분류(RCP_WAY2·RCP_PAT2)가 `description` 안에
...
/// ```
/// 월계수잎과 통후추 등을 사용해 …
/// 조리방법: 굽기 | 요리종류: 반찬      ← 칩으로 올린다
/// 1인분 310.8kcal · 탄수화물 9.4g · …  ← 스탯 타일이 따로 있어 버린다
/// 해시태그: 저염간장                   ← 이름과 달리 주재료라 버린다
/// ```
library;
...
class RecipeTags {
  ...
  /// 칩으로 보여줄 분류. 원문 순서를 지킨다(조리방법 → 요리종류).
  final List<String> labels;

  /// 분류·영양 줄을 걷어낸 본래 설명.
  final String body;

  static const _tagLinePrefixes = ['조리방법:', '요리종류:'];

  /// 원문의 '해시태그:' 는 이름과 달리 **주재료**다(가슴살·등심·저염간장…).
  /// 분류 칩과 나란히 두면 '저염간장'이라는 카테고리가 있는 것처럼 읽히는데,
  /// 사전에 그런 태그가 없어 눌러도 걸릴 필터가 없다. 값 자체는 재료 목록에 이미
  /// 나오므로(표본 12건 중 11건) 칩에서만 빼고 본문에서도 지운다.
  static const _mainIngredientLine = '해시태그:';
  ...
  factory RecipeTags.parse(String description) {
    ...
    for (final rawLine in description.split('\n')) {
      ...
      if (line.startsWith(_mainIngredientLine)) continue;
